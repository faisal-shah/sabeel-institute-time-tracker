...
| Topic | Decision |
|---|---|
| Auth | Sign in with Google **only**, restricted to the **@oursabeel.com** Workspace domain (added 2026-07-24; mirrors the sibling kanban app). Enforced server-side: the gen-1 `onUserCreate` function deletes any non-org account outright, and `firestore.rules` blocks a non-org token from self-registering a profile (`isOrgEmail`); the client also passes Google an `hd` hint (UX only). **Every** org account still lands `pending` until an **admin** approves — no domain auto-approval (the approver sees the requester's name and email). |
